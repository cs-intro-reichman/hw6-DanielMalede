...
public class Runigram {
	...
	public static Color[][] flippedVertically(Color[][] image) {
		//// Replace the following statement with your code
		Color[][] flippedImageColors = new Color[image[0].length][image.length];
		for (int i = 0; i < image[0].length; i++) {
			for (int j = 0; j < image.length; j++) {
				flippedImageColors[image[0].length - 1 - i][j] = image[i][j];
			}
		}
		return flippedImageColors;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		//// Replace the following statement with your code
		Color[][] grayImageColors = new Color[image.length][image[0].length];
		for (int i = 0; i < image.length; i++) {
			for (int j = 0; j < image[0].length; j++) {
				grayImageColors[i][j] = luminance(image[i][j]);
			}
		}
		return grayImageColors;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int y = (i * image.length) / (height);
				int x = (j * image[0].length) / (width);
				;
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		//// Replace the following statement with your code
		int redColor = (int) (alpha * c1.getRed() + (1 - alpha) * c2.getRed());
		int greenColor = (int) (alpha * c1.getGreen() + (1 - alpha) * c2.getGreen());
		int blueColor = (int) (alpha * c1.getBlue() + (1 - alpha) * c2.getBlue());
		Color blendedColor = new Color(redColor, greenColor, blueColor);
		return blendedColor;
	}
